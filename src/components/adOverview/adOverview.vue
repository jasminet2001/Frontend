  ...
  <v-app>
    ...
    <v-row class="my-10">
      <v-col cols="3" class="mr-5 mt-9">
        ...
        <v-btn
          :to="'/company/' + adInfo.sender.company.id"
          rounded
          class="btn btn1"
        >
        ...
        <v-dialog transition="dialog-bottom-transition" max-width="600">
          <template v-slot:activator="{ on, attrs }">
            <v-btn rounded class="btn btn2" v-bind="attrs" v-on="on">
          ...
          <template v-slot:default="dialog">
            <v-card>
              <v-toolbar dark>ارسال درخواست همکاری</v-toolbar>
              <v-card-text>
                <v-form>
                  <v-textarea
                    v-model="message"
              ...
              <v-card-actions class="justify-end">
                <v-btn text @click="dialog.value = false">انصراف</v-btn>
                <v-btn
                  class="btn3"
                  @click="
                    () => {
  ...
  methods: {
    ...
    async submit() {
      if (
        ...
        !this.$cookies.get("user").role === "company"
      ) {
      ...
      axios(config)
        ...
        .catch(function () {
          that.$toast.error(
            "مشکلی در ارسال رخ داد! مطمئن شوید با حساب یک شرکت وارد سایت شده اید."
          );
        });
    },
  },
