...
describe('PurchasePlanComponent', () => {
  ...
  it('displays plan titles correctly', () => {
    ...
    cy.get('[name="plans"]').each(($card, index) => {
      cy.wrap($card).find('[name="plan-card-desktop"]').should('contain', 'Title ' + (index + 1));
    });
  });
  ...
  it('changes the background color of the plan card on hover', () => {
    ...
    cy.intercept('GET', '**/user/BuyPlan/0').as('buyPlan');
    cy.visit(baseData.host + '/company/payment');
    cy.get('[name="plan-card-desktop"]').first().trigger('mouseover').should('have.css', 'background-color', 'rgb(123, 45, 67)');
    cy.get('.[name="plan-card-desktop"]').first().trigger('mouseout').should('have.css', 'background-color', 'rgb(0, 0, 0)');
  });
});
